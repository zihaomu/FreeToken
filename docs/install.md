...
`libthrust-dev` is needed only when the native GGUF kernels are JIT-compiled;
installing it up front keeps every shipped kernel path available.

Do not install the CUDA-only `accel` extra on ROCm. FreeToken selects its Triton
fallback kernels automatically. Multi-GPU runs automatically use PyTorch's
ROCm/RCCL-backed `nccl` process group because the custom PyNCCL path is NVIDIA-only.
